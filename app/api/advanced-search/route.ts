...
async function fetchTCGPlayerDataFromTCGAPI(title: string, cardId: string | null, cardNumber: string | null, set: string | null, language = 'en') {
  try {
    ...
    const cardMarketPrices = cardData?.pricing?.cardmarket;

    // Extract TCGPlayer variants
    const tcgVariants: any = {};
    let price = 'N/A';
    let priceType = 'N/A';

    if (tcgPrices?.holofoil?.marketPrice) {
        tcgVariants.holofoil = tcgPrices.holofoil.marketPrice;
        if (price === 'N/A') {
            price = tcgPrices.holofoil.marketPrice;
            priceType = 'Holofoil Market';
        }
    }
    if (tcgPrices?.reverseHolofoil?.marketPrice) {
        tcgVariants.reverseHolofoil = tcgPrices.reverseHolofoil.marketPrice;
        if (price === 'N/A') {
            price = tcgPrices.reverseHolofoil.marketPrice;
            priceType = 'Reverse Holofoil Market';
        }
    }
    if (tcgPrices?.normal?.marketPrice) {
        tcgVariants.normal = tcgPrices.normal.marketPrice;
        if (price === 'N/A') {
            price = tcgPrices.normal.marketPrice;
            priceType = 'Normal Market';
        }
    }

    // If still no price, try any available variant
    if (price === 'N/A') {
        const firstAvailable = Object.values(tcgPrices || {}).find((p: any) => p && p.marketPrice) as any;
        if (firstAvailable) {
    ...
    // Extract CardMarket variants and prices
    const cardMarketVariants: any = {};
    let cardMarketData = 'N/A';
    let cardMarketDataExtracted: any = null;

    // Extract all CardMarket pricing data
    if (cardMarketPrices) {
      cardMarketDataExtracted = { ...cardMarketPrices, currency: 'EUR' };
      
      // Use avg if available (TCGdex uses 'avg', not 'averageSellPrice')
      if (cardMarketPrices.avg) {
        cardMarketData = cardMarketPrices.avg;
      } else {
        // Fall back to the first available price field
        const priceFields = ['trend', 'low', 'avg1', 'avg7', 'avg30'];
        for (const field of priceFields) {
          if (cardMarketPrices[field] !== undefined) {
            cardMarketData = cardMarketPrices[field];
            break;
          }
        }
      }
    ...
    if (price !== 'N/A') {
      ...
      if (Object.keys(tcgVariants).length > 0) {
        result.variants = tcgVariants;
      }

      // Add CardMarket data if available (separate field in the result)
      ...
      if (cardMarketDataExtracted) {
        result.cardmarket = cardMarketDataExtracted;
        console.log(`✅ CardMarket data found (no TCGPlayer price):`, cardMarketDataExtracted);
      }
      return result;
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch exchange rate and convert prices to INR
    let currentExchangeRate = 88.72; // Default fallback rate

    // Fetch exchange rates from multiple sources
    let eurToInrRate = 95.5; // Default fallback (EUR is typically worth more than USD)
    const apis = [
      {
        name: 'ExchangeRate-API',
        url: 'https://open.er-api.com/v6/latest/USD',
        parse: (data: any) => ({ usd: data.rates?.INR, eur: data.rates?.INR * 1.076 })
      },
      {
        name: 'Frankfurter USD',
        url: 'https://api.frankfurter.app/latest?from=USD&to=INR',
        parse: (data: any) => ({ usd: data.rates?.INR, eur: null })
      },
      {
        name: 'Frankfurter EUR',
        url: 'https://api.frankfurter.app/latest?from=EUR&to=INR',
        parse: (data: any) => ({ usd: null, eur: data.rates?.INR })
      }
    ];
    ...
    for (const api of apis) {
      try {
        ...
        if (response.ok) {
          ...
          if (rates.usd && !isNaN(rates.usd) && rates.usd > 0) {
            currentExchangeRate = parseFloat(rates.usd);
          }
          if (rates.eur && !isNaN(rates.eur) && rates.eur > 0) {
            eurToInrRate = parseFloat(rates.eur);
          }

          if (currentExchangeRate && eurToInrRate) {
            break; // Got both rates, exit
          }
        }
      } catch (err) {
    ...
    // If we only got USD rate, calculate EUR rate
    if (currentExchangeRate && !eurToInrRate) {
      eurToInrRate = currentExchangeRate * 1.076; // Approximate EUR/USD rate
    }
    ...
    function convertEURtoINR(eurPrice: number | string): number | string {
      if (typeof eurPrice === 'string' && eurPrice.toLowerCase() === 'n/a') {
        return 'N/A';
      }
      return Number(eurPrice) * eurToInrRate;
    }

    // Format INR price with ₹ symbol and commas (whole numbers only)
    function formatINR(inrPrice: number | string): string {
      if (inrPrice === 'N/A') {
        return 'N/A';
      }
      return `₹${Math.round(Number(inrPrice)).toLocaleString('en-IN')}`;
    }

    // Convert pricing data to INR
    function convertPricingToINR(pricingData: any): any {
      if (!pricingData) return pricingData;

      // Check if this is EUR pricing (CardMarket)
      const isEUR = pricingData.currency === 'EUR';
      const convertFunc = isEUR ? convertEURtoINR : convertToINR;

      const converted: any = {
        ...pricingData,
        averagePrice: typeof pricingData.averagePrice === 'number'
          ? formatINR(convertFunc(pricingData.averagePrice))
          : pricingData.averagePrice,
        // Also convert eBay listings if present (always USD)
        listings: pricingData.listings?.map((listing: any) => ({
          ...listing,
          sale_price: typeof listing.sale_price === 'number'
            ? parseFloat(convertToINR(listing.sale_price) as string)
            : listing.sale_price
        }))
      };

      // Process detailed variant pricing (holofoil, reverseHolofoil, normal with lowPrice, midPrice, etc.)
      // Handle tcgPlayer variant pricing
      if (converted.tcgPlayer) {
        // Define the pricing variants to process for tcgPlayer (both camelCase and dash formats)
        const tcgVariants = [
        ...
        for (const variant of tcgVariants) {
          // Check for original format first, then alternate
          const variantKey = converted.tcgPlayer[variant.original] ? variant.original : converted.tcgPlayer[variant.alternate] ? variant.alternate : null;
          
          if (variantKey && converted.tcgPlayer[variantKey]) {
            const variantData = { ...converted.tcgPlayer[variantKey] };
            ...
            for (const field of priceFields) {
              if (typeof variantData[field] === 'number') {
                variantData[field] = formatINR(convertFunc(variantData[field]));
              }
            }
            
            converted.tcgPlayer[variantKey] = variantData;
          }
        }
      }
      
      // Handle cardmarket variant pricing
      if (converted.cardmarket) {
        // Define the pricing variants to process for cardmarket (both camelCase and dash formats)
        ...
        for (const variant of cardmarketVariants) {
          // Check for original format first, then alternate
          const variantKey = converted.cardmarket[variant.original] ? variant.original : converted.cardmarket[variant.alternate] ? variant.alternate : null;
          
          if (variantKey && converted.cardmarket[variantKey]) {
            const variantData = { ...converted.cardmarket[variantKey] };
            ...
            converted.cardmarket[variantKey] = variantData;
          }
        }
      }

      // Convert variants if present (for backward compatibility)
      if (pricingData.variants) {
        converted.variants = {};
        for (const [key, value] of Object.entries(pricingData.variants)) {
          converted.variants[key] = typeof value === 'number'
            ? formatINR(convertFunc(value))
            : value;
        }
      }
    ...
    // Extract CardMarket data from TCGPlayer response if present
    const cardMarketData = finalTCGPlayerData.cardmarket
      ? convertPricingToINR(finalTCGPlayerData.cardmarket)
      : null;
    ...
    const result = {
      ...
      language,
      ebay: convertPricingToINR(ebayData),
      pokemonPriceTracker: convertPricingToINR(pokemonPriceTrackerData),
      tcgPlayer: convertPricingToINR(finalTCGPlayerData),
